...
def __launch_net_process(debug_mode, monitor_system, project_name, log_type, interfaces, patron_introducido, custome_filters='', solo_resumen=True): 
    try:
        lg = NetSniffer(debug_mode, monitor_system, project_name, log_type, interfaces, patron_introducido, custome_filters, solo_resumen)
        lg.comenzar()
    except KeyboardInterrupt:
        print('Proceso interrumpido por teclado')
...
def __get_current_log_patterns(log_type, log_path): 
    try:
        ...
        if log_type == '':
            print('No hay ningun patron introducido')
        else:
...
if __name__ == '__main__':
    ...
    #     print(config.sections())
        ...
        patron_introducido = config.ConfigSectionMap('App')['log_type']
        ...
        __launch_net_process(debug_mode, envio_de_datos, project_name, log_type, interfaces, patron_introducido, custome_filters, net_summarize)
    except KeyboardInterrupt or EOFError:
        print('Programa interrumpido mediante teclado')        
    print('----------------')
